...
public class GameItem implements AdapterItem<NewsInfo> {
    ...
    public void handleData(NewsInfo data, int pos) {
        mGameVm.setPic(BitmapFactory.decodeResource(b.getRoot().getResources(), data.picResIdArr[0]));
        mGameVm.setTitle(data.title);
        mGameVm.setIcon(BitmapFactory.decodeResource(b.getRoot().getResources(), data.picResIdArr[1]));
